...
from src.callback_data import CreationCallBackData, StartGameCallBack, DeleteGameCallBackData
from src.callback_data.creator import DisplayPlayersCallBack, EditGameCallBackData
...
from src.keyboards import data_to_write, creator_keyboard
...
@creator_game_router.message(Command("manage_game_as_creator"))
async def create_game_handler(message: Message, state: FSMContext) -> None:
    await message.answer(text="Выберите действие", reply_markup=creator_keyboard)


@creator_game_router.callback_query(CreationCallBackData.filter(F.button_name == "create_game"))
...
@creator_game_router.callback_query(StartGameCallBack.filter(F.button_name == "start_game"))
...
@creator_game_router.callback_query(DeleteGameCallBackData.filter(F.button_name == "delete_game"))
async def delete_game_handler(call: CallbackQuery, callback_data: DeleteGameCallBackData, state: FSMContext):
    await state.clear()
    await state.set_state(DeleteGameState.game_name)
    await call.message.answer("Введите идентификтор игры")
...
@creator_game_router.callback_query(DisplayPlayersCallBack.filter(F.button_name == "display_players"))
async def display_connected_players(call: CallbackQuery, callback_data: DeleteGameCallBackData, state: FSMContext):
    await state.clear()
...
@creator_game_router.callback_query(EditGameCallBackData.filter(F.button_name == "edit_game"))
async def change_players_number(call: CallbackQuery, callback_data: EditGameCallBackData, state: FSMContext):
    await state.clear()
